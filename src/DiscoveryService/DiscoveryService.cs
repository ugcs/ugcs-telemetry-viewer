...
namespace UGCS.SsdpDiscoveryService
{
    ...
    {
        private readonly SsdpAgent _ssdpAgent;

        private const string DEFAULT_MULTICAST_IP = "239.198.46.46";
        private const int DEFAULT_PORT = 1991;

        private readonly log4net.ILog _log = log4net.LogManager.GetLogger(typeof(DiscoveryService));
        ...
        {
            _serviceTimer = new System.Timers.Timer(DEAD_SSDP_CHECK_INTERVAL);
            _serviceTimer.Elapsed += onServiceTimer;
        ...
        public Task<Uri> TryFoundAsync(string serviceType, TimeSpan? timeout = null)
        {
            if (timeout.HasValue && timeout.Value <= TimeSpan.Zero)
                throw new ArgumentException("Value must be greater then zero.", nameof(timeout));


            return TryFoundInternalAsync(serviceType, timeout);
        }

        private async Task<Uri> TryFoundInternalAsync(string serviceType, TimeSpan? timeout)
        {
            Uri uri = null;
        ...
        public void TryFound(string serviceType, ServiceUrlEventHandler receiveUrlHandler)
        {
            if (String.IsNullOrEmpty(serviceType))
                throw new ArgumentException(String.Format("{0}: must be not null and not empty", nameof(serviceType)));
            if (receiveUrlHandler == null)
                throw new ArgumentNullException(nameof(receiveUrlHandler));

            if (_listeners.TryGetValue(serviceType, out var serviceUrlEventHandlers))
            {
                if (!serviceUrlEventHandlers.Contains(receiveUrlHandler))
                    serviceUrlEventHandlers.Add(receiveUrlHandler);
                else
                {
                ...
                Parallel.ForEach(foundServices, (service) =>
                {
                    foreach (var location in service.Value)
                    {
                        if (location.Value < DateTimeOffset.UtcNow.ToUnixTimeSeconds() - TIMEOUT_SEC
                            && service.Value.TryRemove(location.Key, out var val))
                        {
